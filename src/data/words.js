export const data = [
	{ name: 'Давид', rate: '1' },
	{ name: 'Откровение', rate: '1' },
	{ name: 'Судьи', rate: '1' },
	{ name: 'Моисей', rate: '1' },
	{ name: 'Эдемский сад', rate: '1' },
	{ name: 'Меч', rate: '1' },
	{ name: 'Ангел', rate: '1' },
	{ name: 'Ковчег', rate: '1' },
	{ name: 'Содом', rate: '1' },
	{ name: 'Израиль', rate: '1' },
	{ name: 'Фараон', rate: '1' },
	{ name: 'Иосиф', rate: '1' },
	{ name: 'Библия', rate: '1' },
	{ name: 'Служение', rate: '1' },
	{ name: 'Армагеддон', rate: '1' },
	{ name: 'Новый мир', rate: '1' },
	{ name: 'Иисус', rate: '1' },
	{ name: 'Журнал', rate: '1' },
	{ name: 'Павел', rate: '1' },
	{ name: 'Святой дух', rate: '1' },
	{ name: 'Каин', rate: '1' },
	{ name: 'Авраам', rate: '1' },
	{ name: 'Бытие', rate: '1' },
	{ name: 'Кувшин', rate: '1' },
	{ name: 'Овца', rate: '1' },
	{ name: 'Пустыня', rate: '1' },
	{ name: 'Пророк', rate: '1' },
	{ name: 'Храм', rate: '1' },
	{ name: 'Крещение', rate: '1' },
	{ name: 'Самсон', rate: '1' },
	{ name: 'Манна', rate: '1' },
	{ name: 'Иона', rate: '1' },
	{ name: 'Адам', rate: '1' },
	{ name: 'Ева', rate: '1' },
	{ name: 'Запретный плод', rate: '1' },
	{ name: 'Ной', rate: '1' },
	{ name: 'Соломон', rate: '1' },
	{ name: 'Священник', rate: '1' },
	{ name: 'Потоп', rate: '1' },
	{ name: 'Любовь', rate: '1' },
	{ name: 'Голиаф', rate: '1' },
	{ name: 'Саул', rate: '1' },
	{ name: 'Конгресс', rate: '1' },
	{ name: 'Создатель', rate: '1' },
	{ name: 'Псалмопевец', rate: '2' },
	{ name: 'Римляне', rate: '1' },
	{ name: 'Вавилон', rate: '1' },
	{ name: 'Пионер', rate: '1' },
	{ name: 'Старейшина', rate: '1' },
	{ name: 'Помазанник', rate: '1' },
	{ name: 'Ваал', rate: '1' },
	{ name: 'Вирсавия', rate: '1' },
	{ name: 'Сатана', rate: '1' },
	{ name: 'Серафим', rate: '3' },
	{ name: 'Левиты', rate: '2' },
	{ name: 'Раав', rate: '2' },
	{ name: 'Жертвенник', rate: '2' },
	{ name: 'Притча', rate: '2' },
	{ name: 'Город-убежище', rate: '2' },
	{ name: 'Ниневия', rate: '2' },
	{ name: 'Лот', rate: '2' },
	{ name: 'Руфь', rate: '2' },
	{ name: 'Эсфирь', rate: '2' },
	{ name: 'Иоанн', rate: '2' },
	{ name: 'Михаил', rate: '2' },
	{ name: 'Мегиддо', rate: '3' },
	{ name: 'Зевс', rate: '2' },
	{ name: 'Тимофей', rate: '2' },
	{ name: 'Тарс', rate: '2' },
	{ name: 'Навал', rate: '2' },
	{ name: 'Навуходоносор', rate: '2' },
	{ name: 'Аведнего', rate: '2' },
	{ name: 'Евнух', rate: '2' },
	{ name: 'Миссионер', rate: '2' },
	{ name: 'Галаад', rate: '2' },
	{ name: 'Вефиль', rate: '2' },
	{ name: 'Школа благовестников царства', rate: '2' },
	{ name: 'Теократия', rate: '2' },
	{ name: 'Коринф', rate: '2' },
	{ name: 'Афины', rate: '2' },
	{ name: 'Иордан', rate: '2' },
	{ name: 'Синедрион', rate: '2' },
	{ name: 'Фарисей', rate: '2' },
	{ name: 'Назарей', rate: '2' },
	{ name: 'Десятина', rate: '2' },
	{ name: 'Десятиградие', rate: '2' },
	{ name: 'Иерусалим', rate: '2' },
	{ name: 'Жребий', rate: '1' },
	{ name: 'Ефод', rate: '3' },
	{ name: 'Ефа', rate: '3' },
	{ name: 'Динарий', rate: '2' },
	{ name: 'Исайя', rate: '2' },
	{ name: 'Херувим', rate: '2' },
	{ name: 'Легион', rate: '2' },
	{ name: 'Центурион', rate: '2' },
	{ name: 'Пилат', rate: '2' },
	{ name: 'Ур', rate: '2' },
	{ name: 'Иерихон', rate: '2' },
	{ name: 'Авессалом', rate: '2' },
	{ name: 'Мафусал', rate: '3' },
	{ name: 'Гог из Магога', rate: '2' },
	{
		name: 'Идол',
		rate: '1',
	},
	{
		name: 'Видение',
		rate: '1',
	},
	{
		name: 'Посох',
		rate: '1',
	},
	{
		name: 'Первосвященник',
		rate: '2',
	},
	{
		name: 'Аарон',

		rate: '1',
	},
	{
		name: 'Далида',

		rate: '2',
	},
	{
		name: 'Праща',

		rate: '2',
	},
	{
		name: 'Калека',

		rate: '2',
	},
	{
		name: 'Прокаженный',

		rate: '1',
	},
	{
		name: 'Пядь',

		rate: '2',
	},
	{
		name: 'Кротость',

		rate: '1',
	},
	{
		name: 'Мужество',

		rate: '2',
	},
	{
		name: 'Мятеж',

		rate: '2',
	},
	{
		name: 'Накидка',

		rate: '1',
	},
	{
		name: 'Смирение',

		rate: '1',
	},
	{
		name: 'Первенец',

		rate: '1',
	},
	{
		name: 'Ревекка',

		rate: '2',
	},

	{
		name: 'Медное море',

		rate: '2',
	},
	{
		name: 'Фимиам',

		rate: '2',
	},
	{
		name: 'Азазел',

		rate: '3',
	},

	{
		name: 'Темница',

		rate: '1',
	},
	{
		name: 'Колодки',

		rate: '2',
	},
	{
		name: 'Вино',

		rate: '1',
	},
	{
		name: 'Пщеница',

		rate: '1',
	},
	{
		name: 'Жемчужина',

		rate: '1',
	},
	{
		name: 'Пришелец',

		rate: '1',
	},
	{
		name: 'Скрижали',

		rate: '2',
	},
	{
		name: 'Помёт',

		rate: '2',
	},
	{
		name: 'Аваддон',

		rate: '3',
	},

	{
		name: 'Коалиция',

		rate: '2',
	},
	{
		name: 'Ягненок',

		rate: '1',
	},
	{
		name: 'Тетраграмматон',

		rate: '2',
	},
	{
		name: 'Яхве',

		rate: '2',
	},
	{
		name: 'Саранча',

		rate: '1',
	},
	{
		name: 'Родословная',

		rate: '2',
	},
	{
		name: 'Аввакум',

		rate: '2',
	},
	{
		name: 'Летопись',

		rate: '1',
	},
	{
		name: 'Чичивица',

		rate: '2',
	},
	{
		name: 'Инжир',

		rate: '2',
	},
	{
		name: 'Аметист',

		rate: '2',
	},
	{
		name: 'Фарсис',

		rate: '3',
	},
	{
		name: 'Уборная',

		rate: '2',
	},
	{
		name: 'Саддукей',

		rate: '2',
	},
	{
		name: 'Масорет',

		rate: '3',
	},
	{
		name: 'Соферимы',

		rate: '3',
	},
	{
		name: 'Виноград',

		rate: '1',
	},
	{
		name: 'Завет',

		rate: '1',
	},
	{
		name: 'Обет',

		rate: '2',
	},

	{
		name: 'Топаз',

		rate: '2',
	},
	{
		name: 'Кораллы',

		rate: '1',
	},

	{
		name: 'Лагерь',

		rate: '1',
	},
	{
		name: 'Лепешка',

		rate: '1',
	},
	{
		name: 'Выкуп',

		rate: '1',
	},
	{
		name: 'Лаван',
		info: 'Тесть Иосифа, который заставлял его работать 14 лет',
		rate: '3',
	},

	{
		name: 'Ливан',
		info: 'Плодородная гора, на которой рос лес. Часто упоминался Соломоном',
		rate: '3',
	},

	{
		name: 'Господин',

		rate: '1',
	},
	{
		name: 'Гончар',

		rate: '2',
	},
	{
		name: 'Скороход',

		rate: '2',
	},
	{
		name: 'Плакальщица',

		rate: '2',
	},
	{
		name: 'Склеп',

		rate: '2',
	},

	{
		name: 'Елисей',

		rate: '2',
	},
	{
		name: 'Наследник',

		rate: '1',
	},
	{
		name: 'Книжник',

		rate: '2',
	},
	{
		name: 'Анна',

		rate: '2',
	},
	{
		name: 'Самуил',

		rate: '1',
	},
	{
		name: 'Пророчица',

		rate: '2',
	},
	{
		name: 'Ровоам',

		rate: '2',
	},
	{
		name: 'Енакимы',
		info: 'Высокие люди',
		rate: '3',
	},
	{
		name: 'Рувим',

		rate: '2',
	},
	{
		name: 'Седекия',

		rate: '2',
	},
	{
		name: 'Самария',

		rate: '2',
	},
	{
		name: 'Гефсимания',

		rate: '2',
	},
	{
		name: 'Иосия',

		rate: '2',
	},
	{
		name: 'Манассия',

		rate: '2',
	},
	{
		name: 'Ахав',

		rate: '2',
	},
	{
		name: 'Иезавель',

		rate: '2',
	},
	{
		name: 'Езекия',

		rate: '2',
	},
	{
		name: 'Кир',

		rate: '2',
	},
	{
		name: 'Проконсул',
		info: 'Управлял определенной территорией в римскойимперии',
		rate: '3',
	},
	{
		name: 'Помазание',

		rate: '2',
	},
	{
		name: 'Демон',

		rate: '1',
	},
	{
		name: 'Нил',

		rate: '2',
	},
	{
		name: 'Воскресение',

		rate: '2',
	},
	{
		name: 'Душа',

		rate: '2',
	},
	{
		name: 'Жрец',

		rate: '2',
	},
	{
		name: 'Астролог',

		rate: '2',
	},

	{
		name: 'Broadcasting',

		rate: '2',
	},
	{
		name: 'Ноеминь',

		rate: '2',
	},
	{
		name: 'Магдалина',

		rate: '2',
	},
	{
		name: 'Насмехание',

		rate: '2',
	},
	{
		name: 'Лепта',

		rate: '2',
	},

	{
		name: 'Геена',

		rate: '2',
	},
	{
		name: 'Шеол',

		rate: '2',
	},
	{
		name: 'Феофил',

		rate: '2',
	},
	{
		name: 'Диодема',
		info: 'Это было на рогах зверя',
		rate: '3',
	},
	{
		name: 'Левиафан',

		rate: '2',
	},
	{
		name: 'Нефилим',

		rate: '2',
	},
	{
		name: 'Енох',

		rate: '2',
	},
	{
		name: 'Белтешацца́р',
		info: 'Имя, которое дали Даниилу в Вавилоне',
		rate: '3',
	},
	{
		name: 'Ирод',

		rate: '2',
	},
	{
		name: 'Колодец',

		rate: '1',
	},
	{
		name: 'Раскаяние',

		rate: '2',
	},
	{
		name: 'Раб',

		rate: '2',
	},
	{
		name: 'Талант',

		rate: '2',
	},
	{
		name: 'Дарий',
		info: 'Царь персии, правивший во время Неемии',
		rate: '2',
	},
	{
		name: 'Неемия',
		info: 'Восстановил стену и храм в Иерусалиме',
		rate: '2',
	},
	{
		name: 'Гедеон',

		rate: '3',
	},
	{
		name: 'Корнилий',
		info: 'Первый нееврей, получивший святой дух',
		rate: '3',
	},

	{
		name: 'Понтий Пилат',

		rate: '2',
	},
	{
		name: 'Обрезание',

		rate: '2',
	},
	{
		name: 'Искупление',

		rate: '2',
	},
	{
		name: 'Пленник',

		rate: '2',
	},
	{
		name: 'Шева',
		info: 'Богатая страна, из которой к Соломону пришла царица',
		rate: '3',
	},
	{
		name: 'Сандалии',

		rate: '2',
	},
	{
		name: 'Мешковина',

		rate: '2',
	},
	{
		name: 'Благоухание',
		info: 'Приятный запах',
		rate: '2',
	},
	{
		name: 'Исав',

		rate: '2',
	},
	{
		name: 'Исаак',
		info: 'Сын Авраама',
		rate: '2',
	},
	{
		name: 'Знамение',
		info: 'Чудо, сверхестественное явление',
		rate: '3',
	},
	{
		name: 'Закхей',
		info: 'Залез на дерево, чтобы увидеть Иисуса',
		rate: '3',
	},
	{
		name: 'Шафран',
		info: 'Цветущее растение',
		rate: '3',
	},
	{
		name: 'Ладан',
		info: 'Ароматическая смола, которую астрологи подарили Иисусу',
		rate: '3',
	},
	{
		name: 'Мандрагоры',
		info: 'Плоды,которые ела Рахиль, чтобы забеременеть',
		rate: '3',
	},
	{
		name: 'Ракитник',
		info: 'Дерево, угли которого долго сохраняют тепло',
		rate: '3',
	},
	{
		name: 'Сикомор',
		info: 'На это дерево залез человек, который хотел увидеть Иисуса',
		rate: '3',
	},
	{
		name: 'Гадюка',

		rate: '2',
	},
	{
		name: 'Геф',
		info: 'Филистимский город, из которого был родом Голиаф',
		rate: '3',
	},
	{
		name: 'Прозелит',
		info: 'Чужеземец, который принял иудаизм',
		rate: '3',
	},
	{
		name: 'Привратник',
		info: 'Человек, который стоял у ворот',
		rate: '3',
	},
	{
		name: 'Шекель',
		info: 'Еврейская денежная единица',
		rate: '3',
	},

	{
		name: 'Наместник',
		info: 'Царский представитель',
		rate: '3',
	},
	{
		name: 'Бахрома',
		info: 'Отделка по краям ткани. Ее увеличивали себе фаресеи',
		rate: '2',
	},
	{
		name: 'Бурдюк',
		info: 'Фляга из верблюжьей кожи',
		rate: '3',
	},
	{
		name: 'Патмос',
		info: 'Остров, на котором сидел в заключении Павел',
		rate: '3',
	},
	{
		name: 'Месопотамия',
		info: 'Местность между реками Тигр и Еврат',
		rate: '3',
	},
	{
		name: 'Синай',
		info: 'Гора,на которой Моисей провел 40 дней',
		rate: '3',
	},
	{
		name: 'Ионафан',

		rate: '2',
	},
	{
		name: 'Измаил',
		info: 'Первый сын Авраама',
		rate: '3',
	},
	{
		name: 'Ездра',
		info: 'Переписчик во время Неемии',
		rate: '3',
	},
	{
		name: 'Гедеон',
		info: 'У него было войско — 300 человек',
		rate: '3',
	},
	{
		name: 'Корей',
		info: 'Хотел занять место Моисея',
		rate: '2',
	},
	{
		name: 'Финеес',
		info: 'Один из сыновей Илия',
		rate: '3',
	},
	{
		name: 'Аполлос',
		info: 'Христианин обладавший красноречием',
		rate: '3',
	},
	{
		name: 'Мелхиседек',
		info: 'Первосвященник',
		rate: '3',
	},
	{
		name: 'Пятидесятница',
		info: 'В этот день произошло излитие святого духа',
		rate: '3',
	},
	{
		name: 'Иосиф Флавий',
		info: 'Еврейский историк, живший первом веке',
		rate: '3',
	},
	{
		name: 'Агапе',
		info: 'Вид любви, которую мы развиваем в себе усилием воли, по ясным нам причинам',
		rate: '2',
	},
	{
		name: 'Фелиа',
		info: 'Вид любви между друзьями. Нежная и ласковая, возникает сама по себе',
		rate: '3',
	},
	{
		name: 'Сторге́',
		info: 'Вид любви между членами семьи',
		rate: '3',
	},
	{
		name: 'Эрос',
		info: 'Чувственная любовь между мужчиной и женщиной',
		rate: '3',
	},
	{
		name: 'Пур',
		info: 'Жребий',
		rate: '3',
	},
	{
		name: 'Пятикнижие',

		rate: '2',
	},
	{
		name: 'Пакля',
		info: 'Тонкие волокна льна. Самсон разрывал жилы также легко, как этот материал',
		rate: '3',
	},
	{
		name: 'Чертополох',
		info: 'Сорняк, колючее растение',
		rate: '3',
	},
	{
		name: 'Ефремляне',
		info: 'Израильское племя. Потомки одного из сыновей Иосифа',
		rate: '2',
	},
	{
		name: 'Трактат',
		info: 'Маленькая листовка. Предшественник буклетов',
		rate: '2',
	},
	{
		name: 'Мальта',
		info: 'Остров, на котором Павел высадился послекораблекрушения',
		rate: '3',
	},
	{
		name: 'Нейтралитет',

		rate: '2',
	},
	{
		name: 'Эволюция',

		rate: '2',
	},
	{
		name: 'Кадильница',
		info: 'Сосуд для огня, который использовали в священном шатре',
		rate: '3',
	},
	{
		name: 'Рутерфорд',
		info: 'Основатель организациии Исследователей Библии',
		rate: '2',
	},
	{
		name: 'Баалоф',

		rate: '3',
	},
	{
		name: 'Багрянец',

		rate: '2',
	},
	{
		name: 'Бака',

		rate: '3',
	},
	{
		name: 'Баклан',

		rate: '2',
	},
	{
		name: 'Бальзам',

		rate: '1',
	},
	{
		name: 'Бальзамирование',

		rate: '2',
	},
	{
		name: 'Баран',

		rate: '1',
	},
	{
		name: 'Басня',

		rate: '2',
	},
	{
		name: 'Бат',

		rate: '3',
	},
	{
		name: 'Башня',

		rate: '1',
	},
	{
		name: 'Бевай',

		rate: '3',
	},
	{
		name: 'Бегемот',

		rate: '1',
	},
	{
		name: 'Бедный',

		rate: '1',
	},
	{
		name: 'Бедро',

		rate: '1',
	},
	{
		name: 'Беды',

		rate: '1',
	},
	{
		name: 'Беештера',

		rate: '3',
	},
	{
		name: 'Безбрачие',

		rate: '1',
	},
	{
		name: 'Бездна',

		rate: '1',
	},
	{
		name: 'Беззаконие',

		rate: '1',
	},
	{
		name: 'Безумие',

		rate: '1',
	},
	{
		name: 'Безупречность',

		rate: '2',
	},
	{
		name: 'безукоризненность',

		rate: '2',
	},
	{
		name: 'Бела',

		rate: '3',
	},
	{
		name: 'Бенони',

		rate: '3',
	},
	{
		name: 'Беременность',

		rate: '1',
	},
	{
		name: 'Бераха',

		rate: '3',
	},
	{
		name: 'Берилл',

		rate: '2',
	},
	{
		name: 'Берия',

		rate: '3',
	},
	{
		name: 'Бесплодие',

		rate: '2',
	},
	{
		name: 'Беспокойство',

		rate: '1',
	},
	{
		name: 'Бессмертие',

		rate: '1',
	},
	{
		name: 'Бет',

		rate: '3',
	},
	{
		name: 'Беф',

		rate: '3',
	},
	{
		name: 'Библейские рукописи',

		rate: '2',
	},
	{
		name: 'Бирюза',

		rate: '2',
	},
	{
		name: 'Бич',

		rate: '2',
	},
	{
		name: 'Бичевание',

		rate: '2',
	},
	{
		name: 'Благая весть',

		rate: '2',
	},
	{
		name: 'Благовоние',

		rate: '2',
	},
	{
		name: 'Благоговение',

		rate: '2',
	},
	{
		name: 'Благоразумие',

		rate: '2',
	},
	{
		name: 'Благосклонность',

		rate: '2',
	},
	{
		name: 'Благословение',

		rate: '2',
	},
	{
		name: 'Ближний',

		rate: '1',
	},
	{
		name: 'Блоха',

		rate: '1',
	},
	{
		name: 'Блуд',

		rate: '2',
	},
	{
		name: 'Блудница',

		rate: '1',
	},
	{
		name: 'Блюститель порядка',

		rate: '3',
	},
	{
		name: 'Бобы',

		rate: '2',
	},
	{
		name: 'Богатство',

		rate: '1',
	},
	{
		name: 'Богини',

		rate: '2',
	},
	{
		name: 'Богохульство',

		rate: '2',
	},
	{
		name: 'Божественный',

		rate: '1',
	},
	{
		name: 'Болезни',

		rate: '1',
	},
	{
		name: 'Боль',

		rate: '1',
	},
	{
		name: 'Большое дерево',

		rate: '1',
	},
	{
		name: 'Борзая',

		rate: '2',
	},
	{
		name: 'Борода',

		rate: '1',
	},
	{
		name: 'Бохим',

		rate: '3',
	},
	{
		name: 'Брак',

		rate: '1',
	},
	{
		name: 'Браслет',

		rate: '1',
	},
	{
		name: 'Брат',

		rate: '1',
	},
	{
		name: 'Бремя',

		rate: '2',
	},
	{
		name: 'Бритва',

		rate: '1',
	},
	{
		name: 'Бритьё',

		rate: '1',
	},
	{
		name: 'Брожение',

		rate: '2',
	},
	{
		name: 'Бронза',

		rate: '1',
	},
	{
		name: 'Брошь',

		rate: '2',
	},
	{
		name: 'Бубаст',

		rate: '3',
	},
	{
		name: 'Бубен',

		rate: '2',
	},
	{
		name: 'Бул',

		rate: '3',
	},
	{
		name: 'Бумага',

		rate: '1',
	},
	{
		name: 'Бурдюк',

		rate: '2',
	},
	{
		name: 'Бутылочная тыква',

		rate: '2',
	},
	{
		name: 'Бык',

		rate: '1',
	},
	// Пропущено «Давид», т.к. не было указано rate
	{
		name: 'Давида Город',

		rate: '2',
	},
	{
		name: 'Давильня',

		rate: '2',
	},
	{
		name: 'Давир',

		rate: '3',
	},
	{
		name: 'Далида',

		rate: '2',
	},
	{
		name: 'Дамаск',

		rate: '3',
	},
	{
		name: 'Дамасцы',

		rate: '2',
	},
	{
		name: 'Дан',

		rate: '3',
	},
	{
		name: 'Дань',

		rate: '2',
	},
	{
		name: 'Дарий',

		rate: '2',
	},
	{
		name: 'Дарик',

		rate: '3',
	},
	{
		name: 'Даркон',

		rate: '2',
	},
	{
		name: 'Дары милосердия',

		rate: '2',
	},
	{
		name: 'Дары',

		rate: '2',
	},
	{
		name: 'Дверь',

		rate: '1',
	},
	{
		name: 'Дворец',

		rate: '1',
	},
	{
		name: 'Двоюродный брат',

		rate: '1',
	},
	{
		name: 'Девора',

		rate: '3',
	},
	{
		name: 'Девственница',

		rate: '2',
	},
	{
		name: 'Дедушка',

		rate: '1',
	},
	{
		name: 'Деир',

		rate: '3',
	},
	{
		name: 'Демон',

		rate: '1',
	},
	{
		name: 'День Господа',

		rate: '2',
	},
	{
		name: 'День Иеговы',

		rate: '2',
	},
	{
		name: 'День искупления',

		rate: '2',
	},
	{
		name: 'День новолуния',

		rate: '2',
	},
	{
		name: 'День Приготовления',

		rate: '2',
	},
	{
		name: 'День рождения',

		rate: '2',
	},
	{
		name: 'День суда',

		rate: '2',
	},
	{
		name: 'Деньги',

		rate: '1',
	},
	{
		name: 'Деревня',

		rate: '1',
	},
	{
		name: 'Десятина',

		rate: '2',
	},
	{
		name: 'Десять заповедей',

		rate: '2',
	},
	{
		name: 'Десять слов',

		rate: '3',
	},
	{
		name: 'Дети',

		rate: '1',
	},
	{
		name: 'Деторождение',

		rate: '2',
	},
	{
		name: 'Деяния апостолов',

		rate: '2',
	},
	{
		name: 'Диадема',

		rate: '3',
	},
	{
		name: 'Дидрахма',

		rate: '3',
	},
	{
		name: 'Дикий арбуз',

		rate: '2',
	},
	{
		name: 'Дикий бык',

		rate: '2',
	},
	{
		name: 'Дикий козёл',

		rate: '2',
	},
	{
		name: 'Дикий осёл',

		rate: '2',
	},
	{
		name: 'Дикобраз',

		rate: '2',
	},
	{
		name: 'Димас',

		rate: '3',
	},
	{
		name: 'Дина',

		rate: '2',
	},
	{
		name: 'Динарий',

		rate: '2',
	},
	{
		name: 'Добро',

		rate: '1',
	},
	{
		name: 'Добродетельность',

		rate: '2',
	},
	{
		name: 'Доброта',

		rate: '1',
	},
	{
		name: 'Добыча',

		rate: '1',
	},
	{
		name: 'Додо',

		rate: '2',
	},
	{
		name: 'Дождь',

		rate: '1',
	},
	{
		name: 'Долг',

		rate: '2',
	},
	{
		name: 'Долготерпение',

		rate: '2',
	},
	{
		name: 'Долина',

		rate: '2',
	},
	{
		name: 'Долина реки',

		rate: '2',
	},
	{
		name: 'Род',

		rate: '2',
	},
	{
		name: 'Дома души',

		rate: '3',
	},
	{
		name: 'Дорка',

		rate: '3',
	},
	{
		name: 'Дорога',

		rate: '1',
	},
	{
		name: 'Доспехи',

		rate: '2',
	},
	{
		name: 'Дофан',

		rate: '3',
	},
	{
		name: 'Дочь',

		rate: '1',
	},
	{
		name: 'Драгоценности',

		rate: '1',
	},
	{
		name: 'Драгоценные камни',

		rate: '1',
	},
	{
		name: 'Дракон',

		rate: '1',
	},
	{
		name: 'Драхма',

		rate: '3',
	},
	{
		name: 'Древний Днями',

		rate: '2',
	},
	{
		name: 'Дротик',

		rate: '2',
	},
	{
		name: 'Друг',

		rate: '1',
	},
	{
		name: 'Дубина',

		rate: '2',
	},
	{
		name: 'Дума',

		rate: '2',
	},
	{
		name: 'Дух',

		rate: '2',
	},
	{
		name: 'Душа',

		rate: '2',
	},
	{
		name: 'Душистые травы',

		rate: '2',
	},
	{
		name: 'Душистый тростник',

		rate: '2',
	},
	{
		name: 'Дым',

		rate: '1',
	},
	{
		name: 'Дыхание',

		rate: '2',
	},
	{
		name: 'Дьявол',

		rate: '2',
	},
	{
		name: 'Дядя',

		rate: '2',
	},

	// --- Слова из НОВОГО списка (начиная с «Ваал 1») ---
	{
		name: 'Ваал',

		rate: '1',
	},
	{
		name: 'Вааса',

		rate: '2',
	},
	{
		name: 'Вавилон',

		rate: '1',
	},
	{
		name: 'Вавилон Великий',

		rate: '2',
	},
	{
		name: 'Вавилонская башня',

		rate: '1',
	},
	{
		name: 'Вал',

		rate: '1',
	},
	{
		name: 'Валаам',

		rate: '2',
	},
	{
		name: 'Валак',

		rate: '2',
	},
	{
		name: 'Валтасар',

		rate: '2',
	},
	{
		name: 'Варавва',

		rate: '3',
	},
	{
		name: 'Варак',

		rate: '2',
	},
	{
		name: 'Варвар',

		rate: '2',
	},
	{
		name: 'Вариисус',

		rate: '3',
	},
	{
		name: 'Варнава',

		rate: '1',
	},
	{
		name: 'Варух',

		rate: '2',
	},
	{
		name: 'Варфоломей',

		rate: '2',
	},
	{
		name: 'Васан',

		rate: '3',
	},
	{
		name: 'Вафрий',

		rate: '3',
	},
	{
		name: 'Вафуил',

		rate: '2',
	},
	{
		name: 'Вашти',

		rate: '2',
	},
	{
		name: 'Вдова',

		rate: '1',
	},
	{
		name: 'Вдохновение',

		rate: '1',
	},
	{
		name: 'Веельзевул',

		rate: '2',
	},
	{
		name: 'Великан',

		rate: '1',
	},
	{
		name: 'Великое бедствие',

		rate: '1',
	},
	{
		name: 'Великое множество',

		rate: '1',
	},
	{
		name: 'Великое море',

		rate: '2',
	},
	{
		name: 'Венадад',

		rate: '2',
	},
	{
		name: 'Венец',

		rate: '1',
	},
	{
		name: 'Вениамин',

		rate: '1',
	},
	{
		name: 'Вениамитяне',

		rate: '1',
	},
	{
		name: 'Венок',

		rate: '1',
	},
	{
		name: 'Вера',

		rate: '1',
	},
	{
		name: 'Верблюд',

		rate: '1',
	},
	{
		name: 'Вереника',

		rate: '2',
	},
	{
		name: 'Верёвка',

		rate: '1',
	},
	{
		name: 'Верия',

		rate: '2',
	},
	{
		name: 'Верный раб',

		rate: '1',
	},
	{
		name: 'Верхняя комната',

		rate: '1',
	},
	{
		name: 'Веселеил',

		rate: '2',
	},
	{
		name: 'Весло',

		rate: '1',
	},
	{
		name: 'Вестник',

		rate: '1',
	},
	{
		name: 'Весы',

		rate: '1',
	},
	{
		name: 'Ветвь',

		rate: '1',
	},
	{
		name: 'Ветер',

		rate: '1',
	},
	{
		name: 'Вефиль',

		rate: '1',
	},
	{
		name: 'Вечеря',

		rate: '1',
	},
	{
		name: 'Вещи',

		rate: '1',
	},
	{
		name: 'Веяние',

		rate: '2',
	},
	{
		name: 'Вёсельное судно',

		rate: '2',
	},
	{
		name: 'Взятка',

		rate: '1',
	},
	{
		name: 'Видение',

		rate: '1',
	},
	{
		name: 'Вилдад',

		rate: '2',
	},
	{
		name: 'Вина',

		rate: '1',
	},
	{
		name: 'Вино',

		rate: '1',
	},
	{
		name: 'Виноград',

		rate: '1',
	},
	{
		name: 'Виноградник',

		rate: '2',
	},
	{
		name: 'Виночерпий',

		rate: '2',
	},
	{
		name: 'Вирсавия',

		rate: '1',
	},
	{
		name: 'Вифиния',

		rate: '2',
	},
	{
		name: 'Вифлеем',

		rate: '1',
	},
	{
		name: 'Вифлеемлянин',

		rate: '1',
	},
	{
		name: 'Вифсаида',

		rate: '2',
	},
	{
		name: 'Виффагия',

		rate: '2',
	},
	{
		name: 'Владычество',

		rate: '2',
	},
	{
		name: 'Власт',

		rate: '2',
	},
	{
		name: 'Власти',

		rate: '1',
	},
	{
		name: 'Внутренности',

		rate: '1',
	},
	{
		name: 'Вода',

		rate: '1',
	},
	{
		name: 'Военачальник',

		rate: '1',
	},
	{
		name: 'Вождь',

		rate: '1',
	},
	{
		name: 'Воздаяние',

		rate: '3',
	},
	{
		name: 'Возложение рук',

		rate: '2',
	},
	{
		name: 'Возмездие',

		rate: '2',
	},
	{
		name: 'Возмещение',

		rate: '2',
	},
	{
		name: 'Вознесение',

		rate: '2',
	},
	{
		name: 'Возраст',

		rate: '1',
	},
	{
		name: 'Воин',

		rate: '1',
	},
	{
		name: 'Война',

		rate: '1',
	},
	{
		name: 'Войско',

		rate: '1',
	},
	{
		name: 'Воинство',

		rate: '1',
	},
	{
		name: 'Волк',

		rate: '1',
	},
	{
		name: 'Волосы',

		rate: '1',
	},
	{
		name: 'Волшебник',
		rate: '1',
	},
	{
		name: 'Волынка',
		rate: '2',
	},
	{
		name: 'Вольноотпущенник',

		rate: '2',
	},
	{
		name: 'Вооз',

		rate: '1',
	},
	{
		name: 'Вор',

		rate: '1',
	},
	{
		name: 'Воробей',

		rate: '1',
	},
	{
		name: 'Ворон',

		rate: '1',
	},
	{
		name: 'Ворота',

		rate: '1',
	},
	{
		name: 'Воск',

		rate: '1',
	},
	{
		name: 'Воскресение',

		rate: '1',
	},
	{
		name: 'Воспитатель',

		rate: '1',
	},
	{
		name: 'Воссоздание',

		rate: '2',
	},
	{
		name: 'Восстановление',

		rate: '1',
	},
	{
		name: 'Восток',

		rate: '1',
	},
	{
		name: 'Восхваление',

		rate: '2',
	},
	{
		name: 'Восход',

		rate: '1',
	},
	{
		name: 'Восхождение',

		rate: '2',
	},
	{
		name: 'Враг',

		rate: '1',
	},
	{
		name: 'Врач',

		rate: '1',
	},
	{
		name: 'Время',

		rate: '1',
	},
	{
		name: 'Всевышний',

		rate: '1',
	},
	{
		name: 'Всемогущий',

		rate: '1',
	},
	{
		name: 'Всесожжение',

		rate: '2',
	},
	{
		name: 'Встреча',

		rate: '1',
	},
	{
		name: 'Выкидыш',

		rate: '1',
	},
	{
		name: 'Выкуп',

		rate: '1',
	},
	{
		name: 'Вымогательство',

		rate: '2',
	},
	{
		name: 'Вырезать',

		rate: '1',
	},
	{
		name: 'Высокомерие',

		rate: '1',
	},
	{
		name: 'Высоты',

		rate: '2',
	},
	{
		name: 'Высшие власти',

		rate: '1',
	},
	{
		name: 'Вышивание',

		rate: '1',
	},
	{
		name: 'Жадеит',

		rate: '3',
	},
	{
		name: 'Жадность',

		rate: '1',
	},
	{
		name: 'Жалость',

		rate: '1',
	},
	{
		name: 'Жаровня',

		rate: '2',
	},
	{
		name: 'Жатва',

		rate: '2',
	},
	{
		name: 'Жвачка',

		rate: '1',
	},
	{
		name: 'Жезл',

		rate: '2',
	},
	{
		name: 'Железо',

		rate: '1',
	},
	{
		name: 'Желудок',

		rate: '1',
	},
	{
		name: 'Жемчуг',

		rate: '1',
	},
	{
		name: 'Жена',

		rate: '1',
	},
	{
		name: 'Женщина',

		rate: '1',
	},
	{
		name: 'Жертва',

		rate: '1',
	},
	{
		name: 'Жертвенник',

		rate: '2',
	},
	{
		name: 'Жесты',

		rate: '2',
	},
	{
		name: 'Желчь',

		rate: '2',
	},
	{
		name: 'Жёрнов',

		rate: '2',
	},
	{
		name: 'Живот',

		rate: '1',
	},
	{
		name: 'Животные',

		rate: '1',
	},
	{
		name: 'Жизнь',

		rate: '1',
	},
	{
		name: 'Жилы',

		rate: '2',
	},
	{
		name: 'Жир',

		rate: '1',
	},
	{
		name: 'Житница',

		rate: '3',
	},
	{
		name: 'Жнец',

		rate: '2',
	},
	{
		name: 'Жнивьё',

		rate: '3',
	},
	{
		name: 'Жребий',

		rate: '2',
	},

	{
		name: 'Евеи',

		rate: '2',
	},
	{
		name: 'Евер',

		rate: '3',
	},
	{
		name: 'Евиасаф',

		rate: '3',
	},
	{
		name: 'Евилмеродах',

		rate: '3',
	},
	{
		name: 'Евника',

		rate: '2',
	},

	{
		name: 'Еводия',

		rate: '2',
	},
	{
		name: 'Еврей',

		rate: '1',
	},
	{
		name: 'Еврейские Писания',

		rate: '1',
	},
	{
		name: 'Еврейский язык',

		rate: '1',
	},
	{
		name: 'Евреям письмо',

		rate: '1',
	},
	{
		name: 'Евтих',

		rate: '2',
	},
	{
		name: 'Евфрат',

		rate: '2',
	},
	{
		name: 'Египет',

		rate: '1',
	},
	{
		name: 'Еглон',

		rate: '3',
	},
	{
		name: 'Единородный',

		rate: '2',
	},

	{
		name: 'Екрон',

		rate: '3',
	},
	{
		name: 'Елеазар',

		rate: '2',
	},
	{
		name: 'Елиав',

		rate: '3',
	},
	{
		name: 'Елиаким',

		rate: '3',
	},
	{
		name: 'Елиезер',

		rate: '3',
	},
	{
		name: 'Елизавета',

		rate: '2',
	},
	{
		name: 'Елимелех',

		rate: '3',
	},

	{
		name: 'Елифаз',

		rate: '3',
	},
	{
		name: 'Елкана',

		rate: '2',
	},
	{
		name: 'Еннома долина',

		rate: '2',
	},
	{
		name: 'Енос',

		rate: '3',
	},

	{
		name: 'Ермон',

		rate: '2',
	},
	{
		name: 'Ефрем',

		rate: '3',
	},

	{
		name: 'Ефрон',

		rate: '3',
	},

	{
		name: 'Забава',
		rate: '2',
	},
	{
		name: 'Забота',
		rate: '1',
	},
	{
		name: 'Зависимые города',
		rate: '3',
	},
	{
		name: 'Зависть',
		rate: '1',
	},
	{
		name: 'Завулон',
		rate: '3',
	},
	{
		name: 'Завуф',
		rate: '3',
	},
	{
		name: 'Загадка',
		rate: '2',
	},
	{
		name: 'Загон',
		rate: '2',
	},
	{
		name: 'Закат',
		rate: '1',
	},
	{
		name: 'Закваска',
		rate: '2',
	},
	{
		name: 'Заклинание',
		rate: '1',
	},
	{
		name: 'Заключение под стражу',
		rate: '2',
	},
	{
		name: 'Закон',
		rate: '1',
	},
	{
		name: 'Законодатель',
		rate: '2',
	},
	{
		name: 'Закхей',
		rate: '2',
	},
	{
		name: 'Залог',
		rate: '2',
	},
	{
		name: 'Замок',
		rate: '2',
	},
	{
		name: 'Запад',
		rate: '1',
	},
	{
		name: 'Западня',
		rate: '2',
	},
	{
		name: 'Засада',
		rate: '2',
	},
	{
		name: 'Захария',
		rate: '2',
	},
	{
		name: 'Заяц',
		rate: '1',
	},
	{
		name: 'Звезда',
		rate: '1',
	},
	{
		name: 'Зебра',
		rate: '1',
	},
	{
		name: 'Зеведей',
		rate: '3',
	},
	{
		name: 'Зевс',
		rate: '2',
	},
	{
		name: 'Зелень',
		rate: '2',
	},
	{
		name: 'Зелфа',
		rate: '3',
	},
	{
		name: 'Землевладелец',
		rate: '2',
	},
	{
		name: 'Земледелие',
		rate: '1',
	},

	{ name: 'Лаван', rate: '3' },
	{ name: 'Лавр', rate: '3' },
	{ name: 'Лавсония', rate: '3' },
	{ name: 'Лагерь', rate: '1' },
	{ name: 'Ладан', rate: '2' },
	{ name: 'Ладонь', rate: '1' },
	{ name: 'Лазарь', rate: '2' },
	{ name: 'Лазутчик', rate: '3' },
	{ name: 'Ламед', rate: '3' },
	{ name: 'Ламех', rate: '3' },
	{ name: 'Лань', rate: '1' },
	{ name: 'Ласточка', rate: '1' },
	{ name: 'Латинский язык', rate: '2' },
	{ name: 'Лахис', rate: '3' },
	{ name: 'Лебедь', rate: '1' },
	{ name: 'Лев', rate: '1' },
	{ name: 'Левиафан', rate: '2' },
	{ name: 'Левий', rate: '3' },
	{ name: 'Левиратный брак', rate: '2' },
	{ name: 'Левиты', rate: '2' },
	{ name: 'Легион', rate: '2' },
	{ name: 'Лень', rate: '1' },
	{ name: 'Леопард', rate: '1' },
	{ name: 'Лепёшка', rate: '1' },
	{ name: 'Лес', rate: '1' },
	{ name: 'Лестница', rate: '1' },
	{ name: 'Лесть', rate: '2' },
	{ name: 'Летописец', rate: '2' },
	{ name: 'Летописи', rate: '2' },
	{ name: 'Летучая мышь', rate: '1' },
	{ name: 'Лёд', rate: '1' },
	{ name: 'Лён', rate: '2' },
	{ name: 'Лжепророк', rate: '2' },
	{ name: 'Ливан', rate: '2' },
	{ name: 'Ливанский лес', rate: '3' },
	{ name: 'Ливия', rate: '3' },
	{ name: 'Лидия', rate: '2' },
	{ name: 'Ликаония', rate: '3' },
	{ name: 'Ликия', rate: '3' },
	{ name: 'Лисица', rate: '1' },
	{ name: 'Листра', rate: '2' },
	{ name: 'Литое море (медное море)', rate: '2' },
	{ name: 'Лицемер', rate: '2' },
	{ name: 'Лицо', rate: '1' },
	{ name: 'Личинка', rate: '1' },
	{ name: 'Лия', rate: '2' },
	{ name: 'Лоб', rate: '1' },
	{ name: 'Ловушка', rate: '1' },
	{ name: 'Лог', rate: '3' },
	{ name: 'Ложе', rate: '2' },
	{ name: 'Ложь', rate: '1' },
	{ name: 'Лоида', rate: '2' },
	{ name: 'Локоть', rate: '1' },
	{ name: 'Лот', rate: '2' },
	{ name: 'Лотосовое дерево', rate: '3' },
	{ name: 'Лошадь', rate: '1' },
	{ name: 'Лук', rate: '1' },
	{ name: 'Лука', rate: '1' },
	{ name: 'Луки', rate: '1' },
	{ name: 'Луна', rate: '1' },
	{ name: 'Лучник', rate: '1' },
	{ name: 'Лысина', rate: '2' },
	{ name: 'Лягушка', rate: '1' },
	{ name: 'Кадильница', rate: '2' },
	{ name: 'Казнь', rate: '1' },
	{ name: 'Каиафа', rate: '3' },
	{ name: 'Каин', rate: '1' },
	{ name: 'Календарь', rate: '1' },
	{ name: 'Каменная крошка', rate: '2' },
	{ name: 'Каменотёс', rate: '2' },
	{ name: 'Каменщик', rate: '2' },
	{ name: 'Камень', rate: '1' },
	{ name: 'Камыш', rate: '1' },
	{ name: 'Кандакия', rate: '3' },
	{ name: 'Кандалы', rate: '2' },
	{ name: 'Капернаум', rate: '3' },
	{ name: 'Капитель', rate: '3' },
	{ name: 'Каркас', rate: '2' },
	{ name: 'Кармил', rate: '3' },
	{ name: 'Карп', rate: '1' },
	{ name: 'Карьер', rate: '1' },
	{ name: 'Кассия', rate: '3' },
	{ name: 'Квашня', rate: '2' },
	{ name: 'Кедр', rate: '1' },
	{ name: 'Кедрон', rate: '3' },
	{ name: 'Кенхреи', rate: '3' },
	{ name: 'Керенгаппух', rate: '2' },
	{ name: 'Кесария', rate: '3' },
	{ name: 'Кесарь', rate: '2' },
	{ name: 'Кесиль', rate: '3' },
	{ name: 'Киликия', rate: '3' },
	{ name: 'Кимвал', rate: '3' },
	{ name: 'Кинжал', rate: '1' },
	{ name: 'Кинжальщики', rate: '2' },
	{ name: 'Кипарис', rate: '2' },
	{ name: 'Кипр', rate: '2' },
	{ name: 'Кир', rate: '2' },
	{ name: 'Кирена', rate: '3' },
	{ name: 'Кирпич', rate: '1' },
	{ name: 'Кислев', rate: '2' },
	{ name: 'Кифа', rate: '2' },
	{ name: 'Кишки', rate: '1' },
	{ name: 'Клавдий', rate: '2' },
	{ name: 'Клавдия', rate: '2' },
	{ name: 'Клевета', rate: '2' },
	{ name: 'Клеймо', rate: '2' },
	{ name: 'Клетка', rate: '1' },
	{ name: 'Клопас', rate: '3' },
	{ name: 'Ключ', rate: '1' },
	{ name: 'Клятва', rate: '1' },
	{ name: 'Книга', rate: '1' },
	{ name: 'Книжник', rate: '1' },
	{ name: 'Кнут', rate: '1' },
	{ name: 'Князь', rate: '1' },
	{ name: 'Кобра', rate: '1' },
	{ name: 'Ковчег', rate: '1' },
	{ name: 'Ковчег соглашения', rate: '2' },
	{ name: 'Кодекс', rate: '2' },
	{ name: 'Кожа', rate: '1' },
	{ name: 'Кожевник', rate: '2' },
	{ name: 'Козёл', rate: '1' },
	{ name: 'Козлёнок', rate: '1' },
	{ name: 'Козодой', rate: '2' },
	{ name: 'Колдун', rate: '1' },
	{ name: 'Колено', rate: '1' },
	{ name: 'Колесница', rate: '1' },
	{ name: 'Колесо', rate: '1' },
	{ name: 'Колодец', rate: '1' },
	{ name: 'Колодки', rate: '2' },
	{ name: 'Колокольчик', rate: '1' },
	{ name: 'Колонна', rate: '1' },
	{ name: 'Колоннада', rate: '2' },
	{ name: 'Колоссы', rate: '2' },
	{ name: 'Колчан', rate: '2' },
	{ name: 'Кольцо', rate: '1' },
	{ name: 'Кольчуга', rate: '1' },
	{ name: 'Комар', rate: '1' },
	{ name: 'Конь', rate: '1' },
	{ name: 'Копьеносец', rate: '2' },
	{ name: 'Копьё', rate: '1' },
	{ name: 'Кор', rate: '2' },
	{ name: 'Корабль', rate: '1' },
	{ name: 'Коралл', rate: '1' },
	{ name: 'Корван', rate: '2' },
	{ name: 'Корей', rate: '2' },
	{ name: 'Корзина', rate: '1' },
	{ name: 'Кориандр', rate: '2' },
	{ name: 'Коринф', rate: '2' },
	{ name: 'Корица', rate: '2' },
	{ name: 'Кормилица', rate: '2' },
	{ name: 'Кормушка', rate: '1' },
	{ name: 'Корнилий', rate: '2' },
	{ name: 'Корова', rate: '1' },
	{ name: 'Коршун', rate: '1' },
	{ name: 'Косметика', rate: '1' },
	{ name: 'Костный мозг', rate: '2' },
	{ name: 'Кость', rate: '1' },
	{ name: 'Косуля', rate: '2' },
	{ name: 'Котёл', rate: '1' },
	{ name: 'Кошелёк', rate: '1' },
	{ name: 'Краеугольный камень', rate: '2' },
	{ name: 'Крайняя плоть', rate: '2' },
	{ name: 'Крапива', rate: '1' },
	{ name: 'Краски', rate: '1' },
	{ name: 'Красное дерево', rate: '1' },
	{ name: 'Красное море', rate: '1' },
	{ name: 'Кремень', rate: '2' },
	{ name: 'Крепость', rate: '1' },
	{ name: 'Крест', rate: '1' },
	{ name: 'Крещение', rate: '1' },
	{ name: 'Крисп', rate: '3' },
	{ name: 'Крит', rate: '2' },
	{ name: 'Кровосмешение', rate: '2' },
	{ name: 'Кровь', rate: '1' },
	{ name: 'Крокодил', rate: '1' },
	{ name: 'Крот', rate: '1' },
	{ name: 'Кротость', rate: '1' },
	{ name: 'Крюк', rate: '1' },
	{ name: 'Кувшин', rate: '1' },
	{ name: 'Кузнечик', rate: '1' },
	{ name: 'Кукушка', rate: '1' },
	{ name: 'Купец', rate: '2' },
	{ name: 'Куропатка', rate: '1' },
	{ name: 'Кутёж', rate: '3' },
	{ name: 'Куш', rate: '3' },

	{ name: 'Иаг', rate: '1' },
	{ name: 'Иаиль', rate: '1' },
	{ name: 'Иаков', rate: '1' },
	{ name: 'Иаир', rate: '1' },
	{ name: 'Ива', rate: '1' },
	{ name: 'Игла', rate: '1' },
	{ name: 'Игольное ушко', rate: '1' },
	{ name: 'Игры', rate: '1' },
	{ name: 'Идол', rate: '1' },
	{ name: 'Идолопоклонство', rate: '2' },
	{ name: 'Иегова', rate: '1' },
	{ name: 'Иезавель', rate: '1' },
	{ name: 'Иезекииль', rate: '1' },
	{ name: 'Иеремия', rate: '1' },
	{ name: 'Иерихон', rate: '1' },
	{ name: 'Иероваал', rate: '2' },
	{ name: 'Иеровоам', rate: '2' },
	{ name: 'Иерусалим', rate: '1' },
	{ name: 'Иессей', rate: '1' },
	{ name: 'Известняк', rate: '2' },
	{ name: 'Известь', rate: '2' },
	{ name: 'Изгнание', rate: '1' },
	{ name: 'Исключение', rate: '1' },
	{ name: 'Изгородь', rate: '2' },
	{ name: 'Изготовитель палаток', rate: '1' },
	{ name: 'Изнасилование', rate: '1' },
	{ name: 'Изображение', rate: '1' },
	{ name: 'Изолирование', rate: '1' },
	{ name: 'Израиль', rate: '1' },
	{ name: 'Израильтянин', rate: '1' },
	{ name: 'Изумруд', rate: '1' },
	{ name: 'Изъян', rate: '1' },
	{ name: 'Иисус', rate: '1' },
	{ name: 'Иисус Навин', rate: '1' },
	{ name: 'Иконий', rate: '2' },
	{ name: 'Илий', rate: '2' },
	{ name: 'Илья', rate: '1' },
	{ name: 'Индия', rate: '1' },
	{ name: 'Иней', rate: '1' },
	{ name: 'Инжир', rate: '1' },
	{ name: 'Инструмент', rate: '1' },
	{ name: 'Иоанн', rate: '1' },
	{ name: 'Иов', rate: '1' },
	{ name: 'Иоиль', rate: '2' },
	{ name: 'Иона', rate: '1' },
	{ name: 'Ионафан', rate: '1' },
	{ name: 'Иоппия', rate: '2' },
	{ name: 'Иордан', rate: '1' },
	{ name: 'Иосиф', rate: '1' },
	{ name: 'Иофор', rate: '2' },
	{ name: 'Иохаведа', rate: '2' },
	{ name: 'Ирод', rate: '1' },
	{ name: 'Иродиада', rate: '3' },
	{ name: 'Исаак', rate: '1' },
	{ name: 'Исав', rate: '1' },
	{ name: 'Исаия', rate: '1' },
	{ name: 'Искариот', rate: '1' },
	{ name: 'Исключительная преданность', rate: '2' },
	{ name: 'Искусство', rate: '1' },
	{ name: 'Испания', rate: '1' },
	{ name: 'Исправление', rate: '1' },
	{ name: 'Истечение', rate: '2' },
	{ name: 'Истина', rate: '1' },
	{ name: 'Истолкование', rate: '2' },
	{ name: 'Истребление', rate: '1' },
	{ name: 'Исход', rate: '1' },
	{ name: 'Исцеление', rate: '1' },
	{ name: 'Италия', rate: '1' },
	{ name: 'Иуда', rate: '1' },
	{ name: 'Иудей', rate: '1' },
	{ name: 'Обвинение', rate: '1' },
	{ name: 'Обезглавливание', rate: '1' },
	{ name: 'Обезьяна', rate: '1' },
	{ name: 'Обет', rate: '2' },
	{ name: 'Обжора', rate: '1' },
	{ name: 'Облако', rate: '1' },
	{ name: 'Область', rate: '1' },
	{ name: 'Обличение', rate: '2' },
	{ name: 'Обман', rate: '1' },
	{ name: 'Обоз', rate: '2' },
	{ name: 'Образ', rate: '2' },
	{ name: 'Обращение', rate: '2' },
	{ name: 'Обрезание', rate: '2' },
	{ name: 'Обручение', rate: '2' },
	{ name: 'Обучение', rate: '1' },
	{ name: 'Общение', rate: '1' },
	{ name: 'Овал', rate: '1' },
	{ name: 'Овид', rate: '2' },
	{ name: 'Овод', rate: '2' },
	{ name: 'Овца', rate: '1' },
	{ name: 'Огненное озеро', rate: '1' },
	{ name: 'Огонь', rate: '1' },
	{ name: 'Огурец', rate: '1' },
	{ name: 'Одежда', rate: '1' },
	{ name: 'Одержимость', rate: '2' },
	{ name: 'Ожерелье', rate: '1' },
	{ name: 'Озия', rate: '3' },
	{ name: 'Окно', rate: '1' },
	{ name: 'Оковы', rate: '1' },
	{ name: 'Олень', rate: '1' },
	{ name: 'Олово', rate: '1' },
	{ name: 'Омега', rate: '2' },
	{ name: 'Онан', rate: '3' },
	{ name: 'Оникс', rate: '2' },
	{ name: 'Онисим', rate: '3' },
	{ name: 'Онисифор', rate: '3' },
	{ name: 'Оправдание', rate: '2' },
	{ name: 'Опьянение', rate: '1' },
	{ name: 'Орех', rate: '1' },
	{ name: 'Орёл', rate: '1' },
	{ name: 'Оруженосец', rate: '1' },
	{ name: 'Оружие', rate: '1' },
	{ name: 'Орфа', rate: '3' },
	{ name: 'Осадок', rate: '1' },
	{ name: 'Освящение', rate: '1' },
	{ name: 'Осёл', rate: '1' },
	{ name: 'Осия', rate: '2' },
	{ name: 'Основание', rate: '1' },
	{ name: 'Остаток', rate: '1' },
	{ name: 'Остров', rate: '1' },
	{ name: 'Осуждение', rate: '1' },
	{ name: 'Отвес', rate: '2' },
	{ name: 'Отец', rate: '1' },
	{ name: 'Откровение', rate: '1' },
	{ name: 'Отросток', rate: '2' },
	{ name: 'Отступничество', rate: '2' },
	{ name: 'Офир', rate: '3' },
	{ name: 'Офни', rate: '2' },
	{ name: 'Охозия', rate: '2' },
	{ name: 'Охота', rate: '1' },
	{ name: 'Очищение', rate: '1' },

	{ name: 'Павел', rate: '1' },
	{ name: 'Павлин', rate: '1' },
	{ name: 'Пазуха', rate: '3' },
	{ name: 'Пакля', rate: '3' },
	{ name: 'Паланкин', rate: '3' },
	{ name: 'Палестина', rate: '2' },
	{ name: 'Пальма', rate: '1' },
	{ name: 'Памятник', rate: '1' },
	{ name: 'Памятный склеп', rate: '2' },
	{ name: 'Папирус', rate: '2' },
	{ name: 'Пар', rate: '1' },
	{ name: 'Паралич', rate: '2' },
	{ name: 'Парсин', rate: '3' },
	{ name: 'Пастбище', rate: '1' },
	{ name: 'Пастух', rate: '1' },
	{ name: 'Пастырь', rate: '2' },
	{ name: 'Пастушеская сумка', rate: '2' },
	{ name: 'Пасха', rate: '2' },
	{ name: 'Патмос', rate: '3' },
	{ name: 'Паук', rate: '1' },
	{ name: 'Пахота', rate: '3' },
	{ name: 'Пеликан', rate: '1' },
	{ name: 'Пепел', rate: '2' },
	{ name: 'Первенец', rate: '1' },
	{ name: 'Первородства право', rate: '2' },
	{ name: 'Первосвященник', rate: '2' },
	{ name: 'Пергам', rate: '3' },
	{ name: 'Пергамент', rate: '2' },
	{ name: 'Перевод', rate: '1' },
	{ name: 'Передник', rate: '3' },
	{ name: 'Перепел', rate: '1' },
	{ name: 'Переписчик', rate: '2' },
	{ name: 'Перепись', rate: '2' },
	{ name: 'Перес', rate: '3' },
	{ name: 'Переселение', rate: '2' },
	{ name: 'Перо', rate: '1' },
	{ name: 'Персия', rate: '2' },
	{ name: 'Песнь песней', rate: '1' },
	{ name: 'Песня', rate: '1' },
	{ name: 'Песня восхождения', rate: '2' },
	{ name: 'Песок', rate: '1' },
	{ name: 'Петух', rate: '1' },
	{ name: 'Печаль', rate: '1' },
	{ name: 'Печать', rate: '1' },
	{ name: 'Печень', rate: '1' },
	{ name: 'Печь', rate: '1' },
	{ name: 'Пещера', rate: '1' },
	{ name: 'Пёс', rate: '1' },
	{ name: 'Пётр', rate: '1' },
	{ name: 'Пиво', rate: '1' },
	{ name: 'Пила', rate: '1' },
	{ name: 'Пилат', rate: '2' },
	{ name: 'Пир', rate: '1' },
	{ name: 'Писание', rate: '1' },
	{ name: 'Писарь', rate: '3' },
	{ name: 'Писидия', rate: '3' },
	{ name: 'Письма', rate: '1' },
	{ name: 'Письменность', rate: '2' },
	{ name: 'Пища', rate: '1' },
	{ name: 'Пиявка', rate: '2' },
	{ name: 'Плавильщик', rate: '1' },
	{ name: 'Плата', rate: '1' },
	{ name: 'Платан', rate: '3' },
	{ name: 'Плач', rate: '1' },
	{ name: 'Плач Иеремии', rate: '2' },
	{ name: 'Племя', rate: '1' },
	{ name: 'Плен', rate: '1' },
	{ name: 'Пленник', rate: '2' },
	{ name: 'Плесень', rate: '1' },
	{ name: 'Плеть', rate: '1' },
	{ name: 'Плечо', rate: '1' },
	{ name: 'Пловец', rate: '1' },
	{ name: 'Плоскогорье', rate: '1' },
	{ name: 'Плотник', rate: '2' },
	{ name: 'Плоть', rate: '1' },
	{ name: 'Плуг', rate: '2' },
	{ name: 'Побивание камнями', rate: '2' },
	{ name: 'Побои', rate: '1' },
	{ name: 'Повешение', rate: '2' },
	{ name: 'Повитуха', rate: '3' },
	{ name: 'Повозка', rate: '2' },
	{ name: 'Повязка на лбу', rate: '1' },
	{ name: 'Поговорка', rate: '1' },
	{ name: 'Погребение', rate: '1' },
	{ name: 'Погружение в воду', rate: '1' },
	{ name: 'Подарки', rate: '1' },
	{ name: 'Подземелье', rate: '1' },
	{ name: 'Подземелье со львами', rate: '2' },
	{ name: 'Подневольный труд', rate: '2' },
	{ name: 'Подножие', rate: '2' },
	{ name: 'Подчинение', rate: '2' },
	{ name: 'покорность', rate: '2' },
	{ name: 'Пожертвование', rate: '1' },
	{ name: 'Позор', rate: '1' },
	{ name: 'Поклон', rate: '1' },
	{ name: 'Поклонение', rate: '1' },
	{ name: 'Поколение', rate: '1' },
	{ name: 'Покрывало', rate: '1' },
	{ name: 'Покрывание головы', rate: '2' },
	{ name: 'Покупка', rate: '1' },
	{ name: 'Полба', rate: '3' },
	{ name: 'Поле', rate: '1' },
	{ name: 'Полигамия', rate: '2' },
	{ name: 'Полынь', rate: '2' },
	{ name: 'Помазание', rate: '1' },
	{ name: 'Помазанник', rate: '1' },
	{ name: 'Помёт', rate: '1' },
	{ name: 'Помощь нуждающимся', rate: '1' },
	{ name: 'Понимание', rate: '1' },
	{ name: 'Понтий', rate: '2' },
	{ name: 'Попечение', rate: '2' },
	{ name: 'Порей', rate: '3' },
	{ name: 'Порок', rate: '2' },
	{ name: 'Поручительство', rate: '2' },
	{ name: 'Посвящение', rate: '2' },
	{ name: 'Посвящённое', rate: '2' },
	{ name: 'Поселенец', rate: '2' },
	{ name: 'Последние дни', rate: '1' },
	{ name: 'Послушание', rate: '1' },
	{ name: 'Посол', rate: '2' },
	{ name: 'Посох', rate: '1' },
	{ name: 'Посредник', rate: '3' },
	{ name: 'Пост', rate: '1' },
	{ name: 'Постель', rate: '1' },
	{ name: 'Посторонний', rate: '2' },
	{ name: 'Пот', rate: '1' },
	{ name: 'Потифар', rate: '3' },
	{ name: 'Потомство', rate: '1' },
	{ name: 'Потоп', rate: '1' },
	{ name: 'Похвала', rate: '1' },
	{ name: 'Похороны', rate: '1' },
	{ name: 'Поцелуй', rate: '1' },
	{ name: 'Почки', rate: '1' },
	{ name: 'Пощёчина', rate: '1' },
	{ name: 'Пояс', rate: '1' },
	{ name: 'Праведность', rate: '1' },
	{ name: 'Правитель', rate: '1' },
	{ name: 'Правительство', rate: '2' },
	{ name: 'Праздник посвящения', rate: '3' },
	{ name: 'Праздник трубного звука', rate: '3' },
	{ name: 'Праздник шалашей', rate: '2' },
	{ name: 'Празднование субботы', rate: '2' },
	{ name: 'Прах', rate: '1' },
	{ name: 'Прачечник', rate: '2' },
	{ name: 'Праща', rate: '1' },
	{ name: 'Пращник', rate: '3' },
	{ name: 'Преданность', rate: '1' },
	{ name: 'Предатель', rate: '1' },
	{ name: 'Предвидение', rate: '3' },
	{ name: 'Предзнаменование', rate: '3' },
	{ name: 'Предсказатель будущего', rate: '2' },
	{ name: 'Предшественник', rate: '2' },
	{ name: 'Прелюбодеяние', rate: '2' },
	{ name: 'Пренебрежение', rate: '2' },
	{ name: 'Преображение', rate: '2' },
	{ name: 'Преследование', rate: '1' },
	{ name: 'Пресмыкающиеся', rate: '2' },
	{ name: 'Праздник пресных лепёшек', rate: '2' },
	{ name: 'Престол', rate: '1' },
	{ name: 'Преступление', rate: '1' },
	{ name: 'Приближение к Богу', rate: '2' },
	{ name: 'Прививка', rate: '2' },
	{ name: 'Привлекательность', rate: '2' },
	{ name: 'Привратник', rate: '2' },
	{ name: 'Привязанность', rate: '2' },
	{ name: 'Приговор', rate: '1' },
	{ name: 'Придворный', rate: '2' },
	{ name: 'Призрак', rate: '1' },
	{ name: 'Примеры', rate: '1' },
	{ name: 'Примета', rate: '1' },
	{ name: 'Примирение', rate: '2' },
	{ name: 'Приношения', rate: '2' },
	{ name: 'Природа', rate: '1' },
	{ name: 'Приска', rate: '2' },
	{ name: 'Присутствие', rate: '2' },
	{ name: 'Притвор', rate: '3' },
	{ name: 'Притча', rate: '1' },
	{ name: 'Пришелец', rate: '2' },
	{ name: 'Провидец', rate: '2' },
	{ name: 'Провинция', rate: '2' },
	{ name: 'Прозелит', rate: '2' },
	{ name: 'Прозорливец', rate: '2' },
	{ name: 'Проказа', rate: '1' },
	{ name: 'Проклятие', rate: '1' },
	{ name: 'Проконсул', rate: '3' },
	{ name: 'Проповедник', rate: '1' },
	{ name: 'проповедь', rate: '1' },
	{ name: 'Пророк', rate: '1' },
	{ name: 'Пророчество', rate: '1' },
	{ name: 'Пророчица', rate: '1' },
	{ name: 'Просить милостыню', rate: '2' },
	{ name: 'Просо', rate: '2' },
	{ name: 'Проститутка', rate: '1' },
	{ name: 'Пространство', rate: '2' },
	{ name: 'Проступок', rate: '2' },
	{ name: 'Противник', rate: '1' },
	{ name: 'Проценты', rate: '1' },
	{ name: 'Прощение', rate: '1' },
	{ name: 'Прядение', rate: '2' },
	{ name: 'Птицелов', rate: '1' },
	{ name: 'Птицы', rate: '1' },
	{ name: 'Публий', rate: '3' },
	{ name: 'Пугало', rate: '1' },
	{ name: 'Пуд', rate: '3' },
	{ name: 'Пур', rate: '3' },
	{ name: 'Пурим', rate: '2' },
	{ name: 'Пурпурный цвет', rate: '2' },
	{ name: 'Пустыня', rate: '1' },
	{ name: 'Путеолы', rate: '3' },
	{ name: 'Путь', rate: '2' },
	{ name: 'Пчела', rate: '1' },
	{ name: 'Пшеница', rate: '1' },
	{ name: 'Пыль, прах', rate: '2' },
	{ name: 'Пьянство', rate: '2' },
	{ name: 'Пядь', rate: '2' },
	{ name: 'Пятидесятница', rate: '1' },
	{ name: 'Пятикнижие', rate: '2' },
	{ name: 'Пятка', rate: '1' },

	{ name: 'Магия', rate: '1' },
	{ name: 'Магог', rate: '2' },
	{ name: 'Малахия', rate: '1' },
	{ name: 'Мальта', rate: '2' },
	{ name: 'Манассия', rate: '2' },
	{ name: 'Мандрагора', rate: '3' },
	{ name: 'Манна', rate: '1' },
	{ name: 'Мардохей', rate: '2' },
	{ name: 'Мария', rate: '1' },
	{ name: 'Маслина', rate: '1' },
	{ name: 'Масличная гора', rate: '1' },
	{ name: 'Масло', rate: '1' },
	{ name: 'Масляное дерево', rate: '2' },
	{ name: 'Матфан', rate: '3' },
	{ name: 'Матфей', rate: '1' },
	{ name: 'Мать', rate: '1' },
	{ name: 'Мафусал', rate: '2' },
	{ name: 'Мегиддо', rate: '2' },
	{ name: 'Медведь', rate: '1' },
	{ name: 'Медная змея', rate: '3' },
	{ name: 'Мерзость', rate: '1' },
	{ name: 'Меч', rate: '1' },
	{ name: 'Мешковина', rate: '1' },
	{ name: 'Мёртвое море', rate: '1' },
	{ name: 'Милет', rate: '3' },
	{ name: 'Милка', rate: '2' },
	{ name: 'Милосердие', rate: '1' },
	{ name: 'Мина', rate: '2' },
	{ name: 'Мирма', rate: '3' },
	{ name: 'Михаил', rate: '2' },
	{ name: 'Михей', rate: '2' },
	{ name: 'Могила', rate: '1' },
	{ name: 'Можжевельник', rate: '1' },
	{ name: 'Молитва', rate: '1' },
	{ name: 'Молотильные сани', rate: '2' },
	{ name: 'Молох', rate: '2' },
	{ name: 'Мотыга', rate: '1' },
	{ name: 'Мрамор', rate: '1' },
	{ name: 'Мудрость', rate: '1' },
	{ name: 'Муха', rate: '1' },
	{ name: 'Мыслительные способности', rate: '1' },
	{ name: 'Мякина', rate: '2' },
	{ name: 'Мясо', rate: '1' },
	{ name: 'Мятеж', rate: '1' },
	{ name: 'Навал', rate: '3' },
	{ name: 'Навеки', rate: '1' },
	{ name: 'Навоз', rate: '2' },
	{ name: 'Навин', rate: '1' },
	{ name: 'Навуходоносор', rate: '1' },
	{ name: 'Наглол', rate: '3' },
	{ name: 'Нагрудник', rate: '1' },
	{ name: 'Наёмный работник', rate: '1' },
	{ name: 'Назарет', rate: '2' },
	{ name: 'Назорей', rate: '2' },
	{ name: 'Накидка', rate: '1' },
	{ name: 'Наложница', rate: '1' },
	{ name: 'Намордник', rate: '1' },
	{ name: 'Наместник', rate: '2' },
	{ name: 'Нарыв', rate: '1' },
	{ name: 'Наставление', rate: '2' },
	{ name: 'Наставник', rate: '1' },
	{ name: 'Насыпь', rate: '2' },
	{ name: 'Нафан', rate: '2' },
	{ name: 'Нахона гумно', rate: '3' },
	{ name: 'Начальник храма', rate: '2' },
	{ name: 'Неаполь', rate: '3' },
	{ name: 'Невод', rate: '1' },
	{ name: 'Негев', rate: '1' },
	{ name: 'Недуг', rate: '1' },
	{ name: 'Неемия', rate: '1' },
	{ name: 'Нефег', rate: '3' },
	{ name: 'Нефилимы', rate: '1' },
	{ name: 'Нигер', rate: '3' },
	{ name: 'Нимрод', rate: '2' },
	{ name: 'Ниневия', rate: '1' },
	{ name: 'Нищие', rate: '1' },
	{ name: 'Ноеминь', rate: '3' },
	{ name: 'Ной', rate: '1' },
	{ name: 'Раав', rate: '1' },
	{ name: 'Раб', rate: '1' },
	{ name: 'Рабсак', rate: '3' },
	{ name: 'Разноплемённый народ', rate: '2' },
	{ name: 'Ракитник', rate: '3' },
	{ name: 'Рахиль', rate: '2' },
	{ name: 'Ревекка', rate: '1' },
	{ name: 'Ревностный', rate: '1' },
	{ name: 'Рефаимы', rate: '2' },
	{ name: 'Рибай', rate: '3' },
	{ name: 'Рим', rate: '1' },
	{ name: 'Ровоам', rate: '2' },
	{ name: 'Родовая община', rate: '1' },
	{ name: 'Родос', rate: '2' },
	{ name: 'Рожон', rate: '3' },
	{ name: 'Руфь', rate: '2' },
	{ name: 'Магдалина', rate: '1' },
	{ name: 'Таз', rate: '1' },
	{ name: 'Тайна', rate: '1' },
	{ name: 'Тайная вечеря', rate: '2' },
	{ name: 'Талант', rate: '2' },
	{ name: 'Талифа куми', rate: '3' },
	{ name: 'Танцы', rate: '1' },
	{ name: 'Таракан', rate: '1' },
	{ name: 'Тарс', rate: '3' },
	{ name: 'Тартар', rate: '3' },
	{ name: 'Татуировка', rate: '1' },
	{ name: 'Творение', rate: '1' },
	{ name: 'Театр', rate: '1' },
	{ name: 'Телега', rate: '2' },
	{ name: 'Телёнок', rate: '1' },
	{ name: 'Тело', rate: '1' },
	{ name: 'Телохранитель', rate: '1' },
	{ name: 'Темница', rate: '1' },
	{ name: 'Тень', rate: '1' },
	{ name: 'Терафим', rate: '2' },
	{ name: 'Терновник', rate: '2' },
	{ name: 'Терпение', rate: '2' },
	{ name: 'Тертий', rate: '3' },
	{ name: 'Тесть', rate: '2' },
	{ name: 'Тиберий', rate: '3' },
	{ name: 'Тимофей', rate: '2' },
	{ name: 'Тимофею письма', rate: '3' },
	{ name: 'Тир', rate: '2' },
	{ name: 'Тит', rate: '2' },
	{ name: 'Титу письмо', rate: '3' },
	{ name: 'Тихик', rate: '3' },
	{ name: 'Ткачество', rate: '2' },
	{ name: 'Топаз', rate: '3' },
	{ name: 'Традиция', rate: '2' },
	{ name: 'Трезвость', rate: '1' },
	{ name: 'Трепет', rate: '2' },
	{ name: 'Тритон', rate: '3' },
	{ name: 'Триумфальное шествие', rate: '3' },
	{ name: 'Тростник', rate: '2' },
	{ name: 'Трофим', rate: '3' },
	{ name: 'Туман', rate: '1' },
	{ name: 'Туммим', rate: '3' },
	{ name: 'Тушканчик', rate: '2' },
	{ name: 'Тюрбан', rate: '3' },
	{ name: 'Тюремщик', rate: '2' },
	{ name: 'Тюрьма', rate: '1' },
	{ name: 'Яблоко', rate: '1' },
	{ name: 'Явление', rate: '1' },
	{ name: 'Ягнёнок', rate: '1' },
	{ name: 'Яд', rate: '1' },
	{ name: 'Язва', rate: '2' },
	{ name: 'Язычники', rate: '1' },
	{ name: 'Якорь', rate: '1' },
	{ name: 'Яма', rate: '1' },
	{ name: 'Ярмо', rate: '1' },
	{ name: 'Ярость', rate: '2' },
	{ name: 'Ясень', rate: '3' },
	{ name: 'Яхве', rate: '2' },
	{ name: 'Ячмень', rate: '2' },
	{ name: 'Яшма', rate: '3' },
	{ name: 'Ящерица', rate: '1' },
	{ name: 'Шакал', rate: '2' },
	{ name: 'Шалаш', rate: '1' },
	{ name: 'Шатер', rate: '1' },
	{ name: 'Шатер собрания', rate: '3' },
	{ name: 'Шафран', rate: '2' },
	{ name: 'Шейх', rate: '1' },
	{ name: 'Шеол', rate: '2' },
	{ name: 'Шерсть', rate: '2' },
	{ name: 'Шествие', rate: '3' },
	{ name: 'Шесть', rate: '1' },
	{ name: 'Шёлк', rate: '2' },
	{ name: 'Шибболет', rate: '3' },
	{ name: 'Шило', rate: '1' },
	{ name: 'Шишка', rate: '1' },
	{ name: 'Школа', rate: '1' },
	{ name: 'Шлем', rate: '1' },
	{ name: 'Штукатурка', rate: '2' },
	{ name: 'Соль', rate: '1' },
	{ name: 'Соляной город', rate: '3' },
	{ name: 'Сон', rate: '1' },
	{ name: 'Сорняки', rate: '1' },
	{ name: 'Сосед', rate: '1' },
	{ name: 'Сострадание', rate: '2' },
	{ name: 'Сосуд для огня', rate: '2' },
	{ name: 'Сосуды', rate: '1' },
	{ name: 'Сотворение', rate: '2' },
	{ name: 'Сотник', rate: '2' },
	{ name: 'Соты', rate: '1' },
	{ name: 'Софония', rate: '2' },
	{ name: 'Союз', rate: '2' },
	{ name: 'Спасение', rate: '1' },
	{ name: 'Спаситель', rate: '1' },
	{ name: 'Спиритизм', rate: '2' },
	{ name: 'Сплав золота и серебра', rate: '2' },
	{ name: 'Сплетни', rate: '1' },
	{ name: 'Справедливость', rate: '1' },
	{ name: 'Средства передвижения', rate: '2' },
	{ name: 'Ссора', rate: '1' },
	{ name: 'Стадий', rate: '2' },
	{ name: 'Старейшина', rate: '1' },
	{ name: 'Стекло', rate: '1' },
	{ name: 'Стены', rate: '1' },
	{ name: 'Стефан', rate: '2' },

	{ name: 'Щавель', rate: '2' },
	{ name: 'Щедрость', rate: '2' },
	{ name: 'Щека', rate: '1' },
	{ name: 'Щёлочь', rate: '2' },
	{ name: 'Щипцы для снятия нагара', rate: '3' },
	{ name: 'Щит', rate: '2' },

	{ name: 'Чайка', rate: '1' },
	{ name: 'Чан', rate: '2' },
	{ name: 'Час', rate: '1' },
	{ name: 'Чаша', rate: '1' },
	{ name: 'Человек', rate: '1' },
	{ name: 'Человек беззакония', rate: '2' },
	{ name: 'Червь', rate: '1' },
	{ name: 'Череп', rate: '1' },
	{ name: 'Черепок', rate: '2' },
	{ name: 'Чернила', rate: '1' },
	{ name: 'Чернильный прибор писаря', rate: '3' },
	{ name: 'Чертополох', rate: '2' },
	{ name: 'Чеснок', rate: '1' },
	{ name: 'Честь', rate: '2' },
	{ name: 'Чечевица', rate: '1' },
	{ name: 'Чешуя', rate: '1' },
	{ name: 'Чёрный тмин', rate: '2' },
	{ name: 'Числа', rate: '1' },
	{ name: 'Чистота', rate: '1' },
	{ name: 'Чистый', rate: '1' },
	{ name: 'Чтение', rate: '1' },
	{ name: 'Чудо', rate: '1' },
	{ name: 'Чужеземец', rate: '2' },
	{ name: 'Юбилей', rate: '2' },
	{ name: 'Юг', rate: '1' },
	{ name: 'Юлий', rate: '2' },
	{ name: 'Юлия', rate: '1' },

	{ name: 'Фаддей', rate: '3' },
	{ name: 'Факел', rate: '1' },
	{ name: 'Фамарь', rate: '2' },
	{ name: 'Фараон', rate: '1' },
	{ name: 'Фарисеи', rate: '1' },
	{ name: 'Фарра', rate: '2' },
	{ name: 'Фарсис', rate: '2' },
	{ name: 'Фартук', rate: '1' },
	{ name: 'Февда', rate: '3' },
	{ name: 'Фекоя', rate: '2' },
	{ name: 'Феликс', rate: '2' },
	{ name: 'Феннана', rate: '3' },
	{ name: 'Феофил', rate: '3' },
	{ name: 'Ферезеи', rate: '3' },
	{ name: 'Фессалоника', rate: '2' },
	{ name: 'Фест', rate: '3' },
	{ name: 'Филадельфия', rate: '3' },
	{ name: 'Филактерия', rate: '3' },
	{ name: 'Филимон', rate: '2' },
	{ name: 'Филипп', rate: '2' },
	{ name: 'Филиппы', rate: '2' },
	{ name: 'Филистия', rate: '3' },
	{ name: 'филистимляне', rate: '2' },
	{ name: 'Филолог', rate: '2' },
	{ name: 'Философия', rate: '2' },
	{ name: 'Фимиам', rate: '2' },
	{ name: 'Финеес', rate: '2' },
	{ name: 'Финик', rate: '2' },
	{ name: 'Фисташка', rate: '1' },
	{ name: 'Флейта', rate: '1' },
	{ name: 'Фома', rate: '2' },
	{ name: 'Фригия', rate: '3' },
	{ name: 'Фура', rate: '2' },

	{ name: 'Халдей', rate: '2' },
	{ name: 'Хам', rate: '2' },
	{ name: 'Хамелеон', rate: '1' },
	{ name: 'Ханаан', rate: '2' },

	{ name: 'Харран', rate: '3' },
	{ name: 'Хвала', rate: '2' },
	{ name: 'Хелкия', rate: '3' },
	{ name: 'Херувимы', rate: '2' },
	{ name: 'Хлеб', rate: '1' },
	{ name: 'Хлебное приношение', rate: '2' },
	{ name: 'Хлопок', rate: '2' },
	{ name: 'Хлоя', rate: '2' },
	{ name: 'Холм', rate: '1' },
	{ name: 'Хомер', rate: '3' },
	{ name: 'Храм', rate: '1' },
	{ name: 'Хранилище', rate: '2' },
	{ name: 'Хризолит', rate: '3' },
	{ name: 'Христианин', rate: '1' },
	{ name: 'Христос', rate: '1' },
	{ name: 'Хромота', rate: '2' },
	{ name: 'Хронология', rate: '2' },
	{ name: 'Хрусталь', rate: '2' },

	{ name: 'Убежище', rate: '1' },
	{ name: 'Убийство', rate: '1' },
	{ name: 'Убийца', rate: '1' },
	{ name: 'Угловой камень', rate: '2' },

	{ name: 'Удод', rate: '3' },
	{ name: 'Ужин Господа', rate: '1' },

	{ name: 'Уззий', rate: '3' },
	{ name: 'Уззия', rate: '3' },
	{ name: 'Узы', rate: '1' },

	{ name: 'Украшение', rate: '1' },
	{ name: 'Укроп', rate: '1' },
	{ name: 'Уксус', rate: '1' },

	{ name: 'Улитка', rate: '1' },
	{ name: 'Улица', rate: '1' },

	{ name: 'Ум', rate: '1' },
	{ name: 'Уничтожение', rate: '1' },
	{ name: 'Управление', rate: '1' },
	{ name: 'Управляющий', rate: '1' },
	{ name: 'Упрёк', rate: '1' },
	{ name: 'Упрямство', rate: '1' },
	{ name: 'Уровень', rate: '1' },
	{ name: 'Усердие', rate: '1' },
	{ name: 'Уста', rate: '1' },
	{ name: 'Усы', rate: '1' },
	{ name: 'Усыновление', rate: '1' },
	{ name: 'Утварь', rate: '1' },
	{ name: 'Уток', rate: '3' },
	{ name: 'Утроба', rate: '1' },
	{ name: 'Ученик', rate: '1' },
	{ name: 'Учитель, учить', rate: '1' },

	{ name: 'Эвракилон', rate: '3' },
	{ name: 'Эдем', rate: '1' },
	{ name: 'Эл', rate: '3' },

	{ name: 'Эпидемия', rate: '1' },
	{ name: 'Эпикурейцы', rate: '3' },
	{ name: 'Эпилепсия', rate: '1' },
	{ name: 'Эраст', rate: '3' },
	{ name: 'Эсфирь', rate: '2' },
	{ name: 'Эфес', rate: '1' },

	{ name: 'Эфиопия', rate: '1' },
	{
		name: 'Аарон',
		rate: '2',
	},
	{
		name: 'Аваддон',
		rate: '3',
	},
	{
		name: 'Аввакум',
		rate: '2',
	},
	{
		name: 'Август',
		rate: '2',
	},
	{
		name: 'Авдемелех',
		rate: '3',
	},
	{
		name: 'Авденаго',
		rate: '2',
	},
	{
		name: 'Авдий',
		rate: '3',
	},
	{
		name: 'Авель',
		rate: '1',
	},
	{
		name: 'Авенир',
		rate: '3',
	},
	{
		name: 'Авессалом',
		rate: '2',
	},
	{
		name: 'Авигея',
		rate: '2',
	},
	{
		name: 'Авимелех',
		rate: '3',
	},
	{
		name: 'Авраам',
		rate: '1',
	},
	{
		name: 'Аврам',
		rate: '1',
	},
	{
		name: 'Агат',
		rate: '2',
	},
	{
		name: 'Аггей',
		rate: '2',
	},
	{
		name: 'Агриппа',
		rate: '2',
	},
	{
		name: 'Ад',
		rate: '1',
	},
	{
		name: 'Адам',
		rate: '1',
	},
	{
		name: 'Азария',
		rate: '3',
	},
	{
		name: 'Азия',
		rate: '1',
	},
	{
		name: 'Азот',
		rate: '2',
	},
	{
		name: 'Аист',
		rate: '1',
	},
	{
		name: 'Акация',
		rate: '2',
	},
	{
		name: 'Акила',
		rate: '2',
	},
	{
		name: 'Акр',
		rate: '3',
	},
	{
		name: 'Алебастр',
		rate: '3',
	},
	{
		name: 'Александр',
		rate: '2',
	},
	{
		name: 'Александрия',
		rate: '2',
	},
	{
		name: 'Аллилуйя',
		rate: '2',
	},
	{
		name: 'Алмаз',
		rate: '1',
	},
	{
		name: 'Алоэ',
		rate: '2',
	},
	{
		name: 'Альфа',
		rate: '2',
	},
	{
		name: 'Амалик',
		rate: '3',
	},
	{
		name: 'Аман',
		rate: '3',
	},
	{
		name: 'Аметист',
		rate: '2',
	},
	{
		name: 'Аминадав',
		rate: '3',
	},
	{
		name: 'Аминь',
		rate: '2',
	},
	{
		name: 'Амнистия',
		rate: '3',
	},
	{
		name: 'Амон',
		rate: '3',
	},
	{
		name: 'Амос',
		rate: '3',
	},
	{
		name: 'Ангелы',
		rate: '1',
	},
	{
		name: 'Андрей',
		rate: '1',
	},
	{
		name: 'Анна',
		rate: '1',
	},
	{
		name: 'Антилопа',
		rate: '1',
	},
	{
		name: 'Антиохия',
		rate: '3',
	},
	{
		name: 'Антихрист',
		rate: '2',
	},
	{
		name: 'Апокрифы',
		rate: '3',
	},
	{
		name: 'Аполлион',
		rate: '3',
	},
	{
		name: 'Аполлос',
		rate: '2',
	},
	{
		name: 'Апостол',
		rate: '2',
	},
	{
		name: 'Аппиев рынок',
		rate: '3',
	},
	{
		name: 'Араб',
		rate: '2',
	},
	{
		name: 'Арарат',
		rate: '2',
	},
	{
		name: 'Арбуз',
		rate: '1',
	},
	{
		name: 'Ареопаг',
		rate: '3',
	},
	{
		name: 'Аристарх',
		rate: '3',
	},
	{
		name: 'Армагеддон',
		rate: '2',
	},
	{
		name: 'Ароматическая смола',
		rate: '2',
	},
	{
		name: 'Артаксеркс',
		rate: '2',
	},
	{
		name: 'Артемида',
		rate: '2',
	},
	{
		name: 'Арфа',
		rate: '2',
	},
	{
		name: 'Архангел',
		rate: '2',
	},
	{
		name: 'Археология',
		rate: '2',
	},
	{
		name: 'Архитектура',
		rate: '2',
	},
	{
		name: 'Аса',
		rate: '3',
	},
	{
		name: 'Ассирия',
		rate: '2',
	},
	{
		name: 'Астарта',
		rate: '3',
	},
	{
		name: 'Астрологи',
		rate: '2',
	},
	{
		name: 'Асфальт',
		rate: '2',
	},
	{
		name: 'Афины',
		rate: '2',
	},
	{
		name: 'Ахав',
		rate: '2',
	},
	{
		name: 'Ахимелех',
		rate: '3',
	},
	{
		name: 'Ахитофел',
		rate: '3',
	},
]
